...
def ph_ode (t, x, VMAXAcL, VMAXNHE, VMAXTHCO3, VMAXCA9):
    dxdt = []
    diff_CO2_in_out = PM_CO2 * ( (x[4] / V_extracell) - (x[5] / V_cell) ) * S_cell
	
    nu_MCT_in_out = {
        (2.0 - math.tanh(c2aH_slope * (-math.log10(1000 * x[6] / (V_cell * MW_H))) - c2aH_thr)) * VMAXAcL * MW_H / MW_AcL 
        * (S_cell) * x[6] / ((V_cell) * K_mAcL * MW_H / MW_AcL + x[6])
    }
        
    nu_MCT_out_in = {
        (2.0 - math.tanh(a2cH_slope * (-math.log10(1000 * x[5] / (V_extracell * MW_H))) - a2cH_thr)) * VMAXAcL * MW_H / MW_AcL 
        * (S_cell) * x[5] / (V_extracell * K_mAcL * MW_H / MW_AcL + x[5])
    }
        
    nu_NHE_in_out = {
        SensO2 * SensATP * 0.5 * (1.0 + ((-math.log10(1000 * x[5] / (V_extracell * MW_H))) - pH0_NHE) / (l_NHE + abs((-math.log10(1000 * x[5] / (V_extracell * MW_H))) - pH0_NHE))) 
        * VMAXNHE * (S_cell) * pow(x[6], a) / (pow(V_cell * MW_H * K_mNHE / 1000, a) + pow(x[6], a))
    }
        
    nu_THCO3_out_in = {
        SensATP * (0.5) * 
        (1.0 + math.tanh(l_THCO3 * (-math.log10(1000 * x[5] / (V_extracell * MW_H)) - pHe0_THCO3))) * (0.5) 
    ...
    nu_CA9 = (3.0 + 2.0 * math.tanh(-d_CA9 * SensO2)) * VMAXCA9 * S_cell * x[4] / (V_extracell * K_mCA9 * MW_CO2 / 1000 + x[4])

    # intracellular carbondioxide dynamics
    dxdt[0] = {
        SensO2 * q_O2 * MW_CO2 / MW_O2 # internal rate
            -k1 * x[0] + k2 * x[2] * x[3] * 1000 * MW_CO2 / (V_cell * MW_H * MW_HCO3) # chemical equilibrium
            + diff_CO2_in_out
    }
    ...
    dxdt[1] = {
        SensATP * gAcL * MW_H / MW_AcL * MW_H * V_cell * 1e-6 # internal rate
        + k1 * x[0] * MW_H / MW_CO2 - k2 *x[2] * x[3] * 1000 / (V_cell * MW_HCO3) # chemical equilibrium
        - nu_MCT_in_out + nu_MCT_out_in - nu_NHE_in_out
    }

    # intracellular bicarbonate ions dynamics
    dxdt[2] = {
        k1 * MW_HCO3 / MW_CO2 * x[0] - k2 * x[2] * x[3] * 1000 / (V_cell * MW_H) # chemical equilibrium
        + nu_THCO3_out_in
    }
...
sol = solve_ivp(ph_ode, [0.0, 1.0], x, t_eval=numpy.linspace(0.0, 1.0, num=100000 ), args=(0.0057480000000000005, 3.0900000000000006e-05, 0.0012120000000000002, 5.6820000000000004) )