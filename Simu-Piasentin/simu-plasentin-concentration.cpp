...
int cell(const gsl_vector *x, void *params, gsl_vector *f)
{
    ...
    const double y0 = m_CO2_C-m_CO2_C_old-dt * (
        ...
        //diffusion
        + PM_CO2 * ( m_CO2_c_old - m_CO2_C ) * (4.0 * Pi * pow(r_C, 2.0))
    );
    ...
    const double y1 = m_H_C - m_H_C_old - dt * (
        ...
        + k1 * m_CO2_C - k2 * m_H_C * m_HCO3_C
        //nu_MCT_in->out
        - (2.0 - tanh(c2aH_slope * -log10( m_H_C ) - c2aH_thr)) * VMAXAcL * m_H_C / (K_mAcL + m_H_C) 
        //nu_MCT_out->in
        + (2.0 - tanh(a2cH_slope * (-log10(m_H_c)) - a2cH_thr)) * VMAXAcL * m_H_c / ( K_mAcL + m_H_c)
        //nu_NHE_in->out
        - SensO2 * SensATP * 0.5 * (1.0 + ( -log10( m_H_c ) - pH0_NHE) / (l_NHE + abs((-log10(m_H_c)) - pH0_NHE))) 
        * VMAXNHE * pow(m_H_C, a) / (pow(K_mNHE, a) + pow(m_H_C, a))
    );
    ...
    const double y2 = m_HCO3_C - m_HCO3_C_old - dt * (
        ...
        k1 * m_CO2_C - k2 * m_H_C * m_HCO3_C
        //nu_THCO3_out->in
        + SensATP * (0.5) * (1.0 + tanh(l_THCO3 * (-log10(m_H_c) - pHe0_THCO3))) * (0.5) * (1.0 + tanh(g_THCO3 * (pHi0_THCO3 - (-log10(m_H_C))))) 
        * VMAXTHCO3 * m_HCO3_c / (K_mTHCO3 + m_HCO3_c)
    );
    ...
    const double y3 = m_H_c - m_H_c_old - dt * (
        ...
        * VMAXNHE * pow(m_H_C, a) / (pow(K_mNHE, a) + pow(m_H_C, a))
        //nu_CA9 
        + (3.0 + 2.0 * tanh(-d_CA9 * SensO2)) * VMAXCA9 * m_CO2_c_old / (K_mCA9 + m_CO2_c_old)
    );
