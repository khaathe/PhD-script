...
-k1 * m_CO2_C + k2 * m_H_C * m_HCO3_C * 1000 * MW_CO2 / (4.0 / 3.0 * Pi * pow(r_C, 3.0) * MW_H * MW_HCO3)
    ...
    const double y1 = m_H_C-m_H_C_old-dt * (
                                               ...
                                               + k1 * m_CO2_C * MW_H / MW_CO2-k2 * m_H_C * m_HCO3_C * 1000 / (4.0 / 3.0 * Pi * pow(r_C, 3.0) * MW_HCO3)
                                                     //nu_MCT_in->out
-(2.0-tanh(c2aH_slope * (-log10(1000 * m_H_C / (4.0 / 3.0 * Pi * pow(r_C, 3.0) * MW_H)))- c2aH_thr)) *
                                                     VMAXAcL * MW_H / MW_AcL * (4.0 * Pi * pow(r_C, 2.0)) * m_H_C / ((4.0 / 3.0 * Pi * pow(r_C, 3.0)) * K_mAcL * MW_H / MW_AcL + m_H_C)
                                               //nu_MCT_out->in
                                               + (2.0-tanh(a2cH_slope * (-log10(1000 * m_H_c / (V_c * MW_H)))-a2cH_thr)) * VMAXAcL * MW_H /
                                                     MW_AcL * (4.0 * Pi * pow(r_C, 2.0)) * m_H_c / (V_c * K_mAcL * MW_H / MW_AcL + m_H_c)
                                                     //nu_NHE_in->out
-SensO2 * SensATP * 0.5 * (1.0 + ((-log10(1000 * m_H_c / (V_c * MW_H)))-pH0_NHE) / (l_NHE + abs((-log10(1000 * m_H_c / (V_c * MW_H)))-pH0_NHE))) *
    ...
    //intracellular bicarbonateions dynamics
    const double y2 = m_HCO3_C-m_HCO3_C_old-dt * (
                                                     //chemicalequilibrium
                                                     k1 * MW_HCO3 / MW_CO2 * m_CO2_C-k2 * m_H_C * m_HCO3_C * 1000 / (4.0 / 3.0 * Pi * pow(r_C, 3.0) * MW_H)
                                                     //nu_THCO3_out->in
                                                     + SensATP * (0.5) * (1.0 + tanh(l_THCO3 * (-log10(1000 * m_H_c / (V_c * MW_H))-pHe0_THCO3))) * (0.5) * (1.0 + tanh(g_THCO3 * (pHi0_THCO3-(-log10(1000 * m_H_C / (4.0 / 3.0 * Pi * pow(r_C, .0) * MW_H)))))) * VMAXTHCO3 * (4.0 * Pi * pow(r_C, 2.0)) * m_HCO3_c / (V_c * K_mTHCO3 * MW_HCO3 / 1000 + m_HCO3_c));

    //extracellularhydrogen dynamics
    const double y3 = m_H_c-m_H_c_old-dt * (
                                               //chemicalequilibrium
                                               k1 * m_CO2_c_old * MW_H / MW_CO2-k2 * m_H_c * m_HCO3_c * 1000 / (V_c * MW_HCO3)
                                               //nu_MCT_in->out
                                               + (2.0-tanh(c2aH_slope * (-log10(1000 * m_H_C / (4.0 / 3.0 * Pi * pow(r_C, 3.0) * MW_H)))- c2aH_thr)) * VMAXAcL * MW_H / MW_AcL * (4.0 * Pi * pow(r_C, 2.0)) * m_H_C / ((4.0 / 3.0 * Pi * pow(r_C, 3.0)) * K_mAcL * MW_H / MW_AcL + m_H_C)
                                                     //nu_MCT_out->in
-(2.0-tanh(a2cH_slope * (-log10(1000 * m_H_c / (V_c * MW_H)))-a2cH_thr)) *
                                                     VMAXAcL * MW_H /
                                                     MW_AcL * (4.0 * Pi * pow(r_C, 2.0)) * m_H_c / (V_c * K_mAcL * MW_H / MW_AcL + m_H_c)
                                               //nu_NHE_in->out
                                               + SensATP * SensO2 * 0.5 * (1.0 + ((-log10(1000 * m_H_c / (V_c * MW_H)))-pH0_NHE) / (l_NHE + abs((-log10(1000 * m_H_c / (V_c * MW_H)))-pH0_NHE))) * VMAXNHE * (4.0 * Pi * pow(r_C, 2.0)) * pow(m_H_C, a) / (pow(4.0 / 3.0 * Pi * pow(r_C, 3.0) * MW_H * K_mNHE / 1000, a) + pow(m_H_C, a))
                                               //nu_CA9
                                               + (3.0 + 2.0 * tanh(-d_CA9 * SensO2)) * VMAXCA9 * 4.0 * Pi * pow(r_C, 2.0) * m_CO2_c_old / (V_c * K_mCA9 * MW_CO2 / 1000 + m_CO2_c_old) * MW_H / MW_CO2);
...
-SensATP * (0.5) * (1.0 + tanh(l_THCO3 * (-log10(1000 * m_H_c / (V_c * MW_H))-pHe0_THCO3))) *
                                                         (0.5) * (1.0 + tanh(g_THCO3 * (pHi0_THCO3-(-log10(1000 * m_H_C / (4.0 / 3.0 * Pi * pow(r_C, .0) * MW_H))))))
...
int main(void)
{
    ...
    struct cell_params cell_p = {
        MW_H, MW_CO2, MW_O2, MW_HCO3, MW_AcL, r_C, PM_CO2, gAcL, q_O2, k1, k2,
    ...
    //first term
    outData << "0"
            << "\t" << m_CO2_C_old << "\t" << m_H_C_old << "\t" << m_HCO3_C_old << "\t" << m_H_c_old << "\t" << m_HCO3_c_old << endl;
    outDatapH << "0"
              << "\t"
    ...
    for (j = 1; j < time; j++)
    {
        ...
        if (j % 100 == 0)
        {
            ...
            outData << j * dt << "\t" << gsl_vector_get(s->x, 0) << "\t" << gsl_vector_get(s->x, 1) << "\t"
