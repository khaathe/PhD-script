...
int cell(const gsl_vector *x, void *params, gsl_vector *f)
{
    ...
    double S_cell = (4.0 * Pi * pow(r_C, 2.0));
    double V_cell = 4.0 / 3.0 * Pi * pow(r_C, 3.0); 
    diff_CO2_in_out = PM_CO2 * ( m_CO2_c_old - m_CO2_C ) * S_cell;
    nu_MCT_in_out = (2.0 - tanh(c2aH_slope * (-log10(m_H_C)) - c2aH_thr)) * VMAXAcL / MW_AcL * S_cell * m_H_C / ( K_mAcL + m_H_C);
    nu_MCT_out_in = (2.0 - tanh(a2cH_slope * (-log10(m_H_c)) - a2cH_thr)) * VMAXAcL / MW_AcL * S_cell * m_H_c / (K_mAcL + m_H_c);
    nu_NHE_in_out = 
        SensO2 * SensATP * 
        0.5 * (1.0 + ( (-log10(m_H_c)) - pH0_NHE) / (l_NHE + abs((-log10(m_H_c)) - pH0_NHE))) 
        * VMAXNHE * S_cell * pow(m_H_C, a) / (pow(K_mNHE, a) + pow(m_H_C, a));
    nu_THCO3_out_in = 
        SensATP * 
        (0.5) * (1.0 + tanh(l_THCO3 * (-log10(m_H_c) - pHe0_THCO3))) 
        * (0.5) * (1.0 + tanh(g_THCO3 * (pHi0_THCO3 - (-log10(m_H_C))) ) ) 
        * VMAXTHCO3 * S_cell * m_HCO3_c / (K_mTHCO3 + m_HCO3_c);
    nu_CA9 = (3.0 + 2.0 * tanh(-d_CA9 * SensO2)) * VMAXCA9 * S_cell * m_CO2_c_old / (K_mCA9 + m_CO2_c_old);
    ...
    const double y0 = m_CO2_C-m_CO2_C_old-dt * coeff * (
        //internalrate
        SensO2 * q_O2 * 1e-12 / MW_O2
        //chemicalequilibrium
        -k1 * m_CO2_C + k2 * m_H_C * m_HCO3_C
        //diffusion
        + diff_CO2_in_out
    );
    ...
    const double y3 = m_H_c - m_H_c_old - dt * coeff * (
        //chemical equilibrium
        k1 * m_CO2_c_old - k2 * m_H_c * m_HCO3_c
        //nu_MCT_in->out
        + nu_MCT_in_out
        //nu_MCT_out->in
        - nu_MCT_out_in
...
int main(void)
{
    ...
    const double S_cell = (4.0 * Pi * pow(r_C, 2.0));
    const double V_cell = 4.0 / 3.0 * Pi * pow(r_C, 3.0); 
    ...
    const double VMAXAcL = 9.58 * pow(10,-5)* 1e-12 / (MW_AcL * V_cell * 1e-15 );   // mol/(s*L*um2)
    ...
    const double VMAXNHE = 5.15 * pow(10,-7) * 1e-12 / (4.0 / V_cell * 1e-15);   // mol/(s*L*um2)
    ...
    const double VMAXTHCO3 = 2.02 * pow(10,-5) * 1e-12 / ( MW_HCO3 * V_cell * 1e-15 ); // mol/(s*L*um2)
    ...
    const double VMAXCA9 = 9.47 * pow(10,-2) * 1e-12 / ( V_cell * 1e-15 );   // mol/(s*L)
    ...
    outDatapH << "0" << "\t"
        << -log10(m_H_C_old) << "\t"
        << -log10(m_H_c_old) << "\t" << log10((m_HCO3_C_old * MW_CO2 * k2) / (m_CO2_C_old * MW_HCO3 * k1))
