...
int cell(const gsl_vector *x, void *params, gsl_vector *f)
{
    ...
    diff_CO2_in_out = PM_CO2 * (m_CO2_c_old / V_c - m_CO2_C / V_cell ) * (S_cell);

    nu_MCT_in_out = (2.0 - tanh(c2aH_slope * (-log10(1000 * m_H_C / (V_cell * MW_H))) - c2aH_thr)) * VMAXAcL * MW_H / MW_AcL 
        * (S_cell) * m_H_C / ((V_cell) * K_mAcL * MW_H / MW_AcL + m_H_C);
    nu_MCT_out_in = (2.0 - tanh(a2cH_slope * (-log10(1000 * m_H_c / (V_c * MW_H))) - a2cH_thr)) * VMAXAcL * MW_H / MW_AcL 
        * (S_cell) * m_H_c / (V_c * K_mAcL * MW_H / MW_AcL + m_H_c);
    nu_NHE_in_out = SensO2 * SensATP * 0.5 * (1.0 + ((-log10(1000 * m_H_c / (V_c * MW_H))) - pH0_NHE) / (l_NHE + abs((-log10(1000 * m_H_c / (V_c * MW_H))) - pH0_NHE))) 
        * VMAXNHE * (S_cell) * pow(m_H_C, a) / (pow(V_cell * MW_H * K_mNHE / 1000, a) + pow(m_H_C, a));
    nu_THCO3_out_in = SensATP * (0.5) * (1.0 + tanh(l_THCO3 * (-log10(1000 * m_H_c / (V_c * MW_H)) - pHe0_THCO3))) 
        * (0.5) * (1.0 + tanh(g_THCO3 * (pHi0_THCO3 - (-log10(1000 * m_H_C / (V_cell * MW_H)))))) 
        * VMAXTHCO3 * (S_cell) * m_HCO3_c / (V_c * K_mTHCO3 * MW_HCO3 / 1000 + m_HCO3_c);
    nu_CA9 = (3.0 + 2.0 * tanh(-d_CA9 * SensO2)) * VMAXCA9 * S_cell * m_CO2_c_old  / (V_c * K_mCA9 * MW_CO2 / 1000 + m_CO2_c_old);
    
    double coeff = 1; // Just to increase the uptake/secretion to simulate the effect of a large cell number
    ...
    const double y0 = m_CO2_C-m_CO2_C_old-dt * coeff * (
        ...
        //chemicalequilibrium
         -k1 * m_CO2_C + k2 * m_H_C * m_HCO3_C * 1000 * MW_CO2 / ( V_cell * MW_H * MW_HCO3)
        //diffusion
        + diff_CO2_in_out
    );
    ...
    const double y1 = m_H_C - m_H_C_old - dt *coeff *  (
        ...
        + k1 * m_CO2_C  * MW_H / MW_CO2  - k2 * m_H_C * m_HCO3_C * 1000 / (V_cell * MW_HCO3)
        //nu_MCT_in->out
        - nu_MCT_in_out
        //nu_MCT_out->in
        + nu_MCT_out_in
    ...
    const double y2 = m_HCO3_C - m_HCO3_C_old - dt * coeff * (
        //chemical equilibrium
        k1 * MW_HCO3 / MW_CO2 * m_CO2_C - k2 * m_H_C * m_HCO3_C * 1000 / (V_cell * MW_H)
        //nu_THCO3_out->in
        + nu_THCO3_out_in
    );
...
int main(void)
{
    ...
    // starting conditions
    m_CO2_C_old = 5.39 * pow(10.0, -5) * (V_cell);
    m_H_C_old = pow(10.0, -pH_cell - 3.0) * (V_cell);
    ...
    for (j = 1; j < time; j++)
    {
        ...
        if ( printFullOutput || (j % 100 == 0) )
        {
            ...
            outDatapH << j * dt << "\t"
                << -log10(1000 * gsl_vector_get(s->x, 1) / (V_cell)) << "\t"
