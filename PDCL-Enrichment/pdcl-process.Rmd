...
source("Code/PDCL-Enrichment/plot-result.R")
```
...
```{r}
rank.list <- lapply(deseq2.res.list, function(x){
  rnk <- x$log2FoldChange
  names(rnk) <- row.names(x)
  rnk
})
deseq2.gsea <- lapply(rank.list, run.gsea, reactome.gmt, bioplanet.gmt)
...
collapsed <- collapse.data.by.pathways(gost.enrich, gsea.enrich, pdcl.samples.name, db.source)
collapsed <- assign.categories(collapsed)

collapsed <- collapsed %>% 
  filter(!(undesired | is.na(category)))

collapsed$category <- factor(
  collapsed$category,
  levels = names(word_list)
)

collapsed$pdcl <- factor(
  collapsed$pdcl,
  levels = pdcl.samples.name
)

collapsed.by.method <- collapse.data.by.method(collapsed)
```
...
common_pathway_0.05 <- find.common.pathways(collapsed, threshold)
all_result_0.05 <- collapsed %>% inner_join(common_pathway_0.05, by=c("pathway_id", "pdcl"))
all_result_by_method_0.05 <- collapsed.by.method %>% inner_join(common_pathway_0.05, by=c("pathway_id", "pdcl"))
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.05/plot_gprofiler_vs_gsea.png",
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.05/plot_count_categories.png",
...
count_pathways_0.05 
# ggsave(
#   "~/PhD/Result/PDCL/result_0.05/plot_count_pathways.png",
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.05/heatmap_pathways.png",
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.05/heatmap_categories.png",
...
common_pathway_0.1 <- find.common.pathways(collapsed, threshold)
all_result_0.1 <- collapsed %>% inner_join(common_pathway_0.1, by=c("pathway_id", "pdcl"))
all_result_by_method_0.1 <- collapsed.by.method %>% inner_join(common_pathway_0.1, by=c("pathway_id", "pdcl"))
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.1/plot_gprofiler_vs_gsea.png",
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.1/plot_count_categories.png",
...
count_pathways_0.1 
# ggsave(
#   "~/PhD/Result/PDCL/result_0.1/plot_count_pathways.png",
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.1/heatmap_pathways.png",
...
# ggsave(
#   "~/PhD/Result/PDCL/result_0.1/heatmap_categories.png",
